...
function imgHtml(pai = '', htmlClass = '') {
...
Majiang.View.Shoupai.prototype.redraw = function() {
    ...
    for (var s of ['m','p','s','z']) {
        for (var n = 0; n < this._shoupai._shouli[s].length; n++) {
            ...
            for (var i = 0; i < num; i++) {
                ...
                shouli.append(imgHtml(pai, this._node.hasClass('shoupai--zijia') ? 'pai--zijiaShouli' : ''));
            }
        }
    }
    if (this._shoupai._zimo && this._shoupai._zimo.length == 2) {
        var pai = this._open ? this._shoupai._zimo : null;
        shouli.append('<span class="paiWrapZimo">' + imgHtml(pai, this._node.hasClass('shoupai--zijia') ? 'pai--zijiaShouli' : '') + '</span>');
    }
    ...
    for (var mianzi of this._shoupai._fulou) {
        ...
        if (mianzi.match(/^.(?!(\d)\1).*$/)) {              // 顺子
            ...
            html += '<span class="paiWrapRotate">' + imgHtml(s + nn[0], 'pai--fulou pai--rotate') + '</span>';
            html += imgHtml(s + nn[1], 'pai--fulou') + imgHtml(s + nn[2]), 'pai--fulou';
        }
        if (mianzi.match(/^.(\d)\1\1\1?[\-\+\=]\1?$/)) {    // 刻子 or 明杠子
            ...
            var img    = imgHtml(s + n, 'pai--fulou');
            var img_r0 = imgHtml(s + n, 'pai--fulou pai--rotate');
        ...
        if (mianzi.match(/^.(\d)\1\1\1$/)) {                // 暗杠子
            n = mianzi[1];
            html += imgHtml('', 'pai--fulou') + imgHtml('', 'pai--fulou') + imgHtml('', 'pai--fulou') + imgHtml('', 'pai--fulou');
        }
        html += '</span>';
        fulou.prepend($(html));
    }
...
Majiang.View.He.prototype.redraw = function() {
    ...
    for (var pai of this._he._pai) {
        ...
        else this._node.append($(imgHtml(pai, 'pai--he')));
        i++;
        if (i < 18 && i % 6 == 0) {
  ...
  console.log('[' + this._id +'] <= (zimo, ' + data.zimo + ')');  // for DEBUG
    ...
    if (Majiang.Util.xiangting(this._shoupai) == -1) {
        $('.UI .zimo').bind('click', function(){
            $('.UI span').hide();
            $('.shoupai--zijia .shouli .pai').unbind('click');
    ...
    var self = this;
    $('.shoupai--zijia .shouli .pai').each(function(){
